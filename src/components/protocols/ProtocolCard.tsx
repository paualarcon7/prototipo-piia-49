    ...
    <>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          {!isLocked && <PlayCircle className="w-10 h-10 text-[#D946EF] fill-white/10" />}
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {protocol.tags.map((tag) => (
              <Badge 
                key={tag} 
                variant="secondary" 
                className="bg-white/10 hover:bg-white/20 transition-colors text-[#D946EF]"
              >
                {tag}
              </Badge>
            ))}
          </div>
          <span className="text-sm text-[#D946EF]">{protocol.duration}</span>
        </div>
      </Card>
      ...
      <Dialog open={showLockModal} onOpenChange={setShowLockModal}>
        <DialogContent className="bg-gradient-to-br from-[#1a1a1a] to-[#2a2a2a] border-[#9b87f5]/20">
          <DialogHeader>
            <DialogTitle className="text-[#D946EF]">Protocolo bloqueado</DialogTitle>
