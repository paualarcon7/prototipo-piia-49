    ...
    <>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h2 className="text-xl font-semibold text-white flex items-center gap-2">
              {protocol.title}
              {isLocked && <Lock className="h-5 w-5 text-[#ffcc08]" />}
            </h2>
            {protocol.description && (
              <p className="text-gray-300 mt-1">{protocol.description}</p>
            )}
          </div>
          {!isLocked && <PlayCircle className="w-10 h-10 text-[#ffcc08] fill-white/10" />}
        </div>
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <span className="text-sm text-[#ffcc08]">{protocol.duration}</span>
        </div>
      </Card>

      <Dialog open={showLockModal} onOpenChange={setShowLockModal}>
        <DialogContent className="bg-gradient-to-br from-[#1a1a1a] to-[#2a2a2a] border-[#ffcc08]/20">
          <DialogHeader>
            <DialogTitle className="text-[#ffcc08]">Protocolo bloqueado</DialogTitle>
