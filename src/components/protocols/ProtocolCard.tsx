...
const ProtocolCard = ({ protocol, isLocked = false }: ProtocolCardProps) => {
  ...
  return (
    <>
      <Card 
        key={protocol.id} 
        className={`bg-gradient-to-br from-[#9b87f5]/20 to-[#FF4081]/20 backdrop-blur-sm border-secondary/20 p-6 space-y-4 hover:shadow-lg hover:shadow-[#9b87f5]/10 transition-all duration-300 cursor-pointer relative ${
      ...
      >
        <div className="flex items-center gap-4">
          ...
          {!isLocked && <PlayCircle className="w-10 h-10 text-white fill-[#9b87f5]/10" />}
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {protocol.tags.map((tag) => (
              <Badge 
                key={tag} 
                variant="secondary" 
                className="bg-[#9b87f5]/20 hover:bg-[#9b87f5]/30 transition-colors text-white"
              >
                {tag}
              </Badge>
            ))}
          </div>
          <span className="text-sm text-white">{protocol.duration}</span>
        </div>
      </Card>
      ...
      <Dialog open={showLockModal} onOpenChange={setShowLockModal}>
        <DialogContent className="bg-gradient-to-br from-[#1a1a1a] to-[#2a2a2a] border-[#9b87f5]/20">
          <DialogHeader>
            <DialogTitle className="text-white">Protocolo bloqueado</DialogTitle>
